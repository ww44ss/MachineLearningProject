...
Inspection reveals there are many NA values in the data. A historgram shows that there are really two classes of columns. Those with data and those without. To filter them I choose to eliminate those columns with high numbers (>50%) of NA values.   
...
In general is it hard to see strong dependencies of any particular variable thought there are clear differences between the plots.   
...
I experimented with the number variable. Reducing the number from 4 to 3 decreased the user time about 10% FYI, without apparent reduction in accuracy.   
