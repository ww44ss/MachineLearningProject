...
p <- qplot(Train[,aa], Train[,bb], col=classe, data=Train) + geom_point(size=0.8, alpha=0.1)
p <- p + geom_point(aes(x=varvis[,1],y=varvis[,2],col=classe),size=5,shape=15,data=varvis)
...
The horizontal axis is __`r colnames(Train)[aa]`__ and the verti axiscal is __`r colnames(Train)[bb]`__. The broad distribution of points and proximity of the centroids is obvious. Nevertheless they are distinguishable - particularly the location of the __classe C__ centroid. 
