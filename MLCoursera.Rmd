...
p <- qplot(Train[,aa], Train[,bb], col=classe, data=Train, alpha=.1)
...
The verical axis is __`r colnames(Train)[aa]`__ and the horizontal axis is __`r colnames(Train)[bb]`__. You can see the broad distribution of points and how closely the centroids are. With so many variables it is hard to visualize dependencies easily. Nevertheless, this gives an idea of how "noisy" the data appear and yet how powerful the model is at making distinctions. 
