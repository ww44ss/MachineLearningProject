-
---
title: "Machine Learning Project Plus"
...
###THIS IS TEMP CODE TO SPEED THINGS UP
require(caret)
xx <- createDataPartition(y=training$classe, p=0.20, list=FALSE)
training<-training[xx,]
...
plot(Train$classe,Train[,27])
plot(Train$classe,Train[,31])
plot(Train$classe,Train[,40])
plot(Train$classe,Train[,51])

```

###C: Building a Model

When all else fails, build a model and see what you get. 
...
I experimented with the number variable. Using _number = 3_ decreased the user time about 25% from _number = 4_, without a noticable reduction in accuracy.   
...
varvis
p <- qplot(Train[,aa], Train[,bb], col=classe,data=Train, alpha=0.1)
p <- p + geom_point(aes(x=varvis[,1],y=varvis[,2],col=classe),size=8,shape=4,data=varvis)
...
According to the matrix the model has an accuracy of 99.3% and hence an estimated out of sample error rate of 0.7%. To be honest that blows me away. 

For information this calculation took `r calctime[3]` seconds of elapsed time and `r calctime[2]` seconds of system time on a _MacBook Air_. Every time I ran it my battery level dropped about 5%!!!
